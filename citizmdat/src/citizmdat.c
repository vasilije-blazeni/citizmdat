...
#define VERZIJA	"V0.5  05.11.2018."
 ...
 * i u QNX-u i u GNU/LINUX-u. */
  ...
  " -v   Prikazuje verziju programa i prekida izvrsenje istog.						\n"
	...
	" (+|-)o   Ukljucenje/iskljucenje opsirnog prikaza podataka: polozaja u datoteci u obliku 10-cifrenog			\n"
	...
	{	// zadati program se vrsi za sve zadate datoteke
		...
		while( ( dat = fopen( ime_dat = argv[ indeks_arg ], nacin_otvaranja ) ) == NULL )
		{
			if( errno == ENOENT )
			{
				...
				warn( "GRESKA pri otvaranju datoteke \"%s\"", ime_dat );
				fprintf( stderr, "Da li zelite da je kreirate (d/n)?\n" );
				scanf( "%c", &odziv_char );

				if( odziv_char == 'd' )	// Obrazovace se nova datoteka.
					strcpy( nacin_otvaranja, "w+" );
				else
					return EXIT_FAILURE;
			}
			else
				err( EXIT_FAILURE, "GRESKA pri otvaranju datoteke \"%s\"", ime_dat );
		}
